...
![](ReadmeImages/mapColors.PNG)
![](ReadmeImages/deskSpecs.PNG)
![](ReadmeImages/libraryExample.PNG)
...
1. In 'Map File' file selector, select the pixel map PNG file you want to use for object placement _(you can use the testObjMap.png in the DemoFiles folder for testing)_.
2. In the 'Floor Layer' file selector, select the pixel map PNG file you want to use for floor topography _(you can use the testTopography.png in the DemoFiles for testing)_.
3. Hit the 'Interpret Map' button to get the object selectors for all the pixel colors in the 'Map File'.
4. For each color you want an object to appear on, select an object file (.obj, .fbx) and use the dials to get the transformations you want. The transformed object should appear in the viewport for your reference.
